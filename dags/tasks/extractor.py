...
def create_supplier_dataframe(main_df):
    ...
    try:
        ...
        if main_df.is_empty():
            logger.warning("Main DataFrame is empty - creating empty supplier DataFrame with expected structure.")
...
def create_part_dataframe(main_df):
    ...
    try:
        ...
        # Handle empty DataFrame
        if main_df.is_empty():
            logger.warning("Main DataFrame is empty - creating empty part DataFrame with expected structure.")
            return pl.DataFrame(schema={col: pl.String() for col in required_columns})
...
def create_box_dataframe(main_df):
    ...
    try:
        ...
        # Handle empty DataFrame
        if main_df.is_empty():
            logger.warning("Main DataFrame is empty - creating empty box DataFrame with expected structure.")
            return pl.DataFrame(schema={col: pl.String() for col in required_columns})
...
def create_pallet_dataframe(main_df):
    ...
    try:
        ...
        # Handle empty DataFrame
        if main_df.is_empty():
            logger.warning("Main DataFrame is empty - creating empty pallet DataFrame with expected structure.")
            return pl.DataFrame(schema={col: pl.String() for col in required_columns})
...
def create_model_dataframe(main_df):
    ...
    try:
        ...
        # Handle empty DataFrame
        if main_df.is_empty():
            logger.warning("Main DataFrame is empty - creating empty model DataFrame with expected structure")
            return pl.DataFrame(schema={col: pl.String() for col in required_columns})
...
def create_workshop_dataframe(main_df):
    ...
    try:
        ...
        # Handle empty DataFrame
        if main_df.is_empty():
            logger.warning("Main DataFrame is empty - creating empty workshop DataFrame with expected structure.")
            return pl.DataFrame(schema={col: pl.String() for col in required_columns})
...
def create_line_dataframe(main_df):
    '''
    ...
    try:
        ...
        # Handle empty DataFrame
        if main_df.is_empty():
            logger.warning("Main DataFrame is empty - creating empty line DataFrame with expected structure")
            return pl.DataFrame(schema={col: pl.String() for col in required_columns})
...
def extractor():
    ...
    try:
        ...
        logger.warning("Error reading Excel with default settings: %s. Trying alternative approach.", e)
        # Fallback to pandas if polars didn't read
        try:
            pd_df = pd.read_excel(file_path)
            main_df = pl.from_pandas(pd_df)
            logger.info("Successfully read Excel via pandas fallback")

        except Exception as pd_error:
            logger.error("Failed to read Excel file: %s", pd_error)
            raise
