...
Module Structure:
    ...
    - Column-to-model mapping configuration
    - Composite key handling utilities
    - Caching system with statistics

Composite Key Handling:
    - Boxes: Maps (type, length, width, height) → 'R/N L-W-H' → box_id
    - Pallets: Maps (type, length, width, height) → 'R/N L-W-H' → pallet_id
    Where: R = Returnable, N = Non-returnable
...
Error Handling:
    - Graceful handling of missing mappings (returns None)
    - Input validation for composite keys
    - Comprehensive logging for debugging
    - No exceptions for missing data (only warnings)
...
class MFTObjectMapper:
    ...
    def get_id(
        ...
        if column_name in COMPOSITE_COLUMNS:
